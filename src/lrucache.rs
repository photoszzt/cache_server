...
// Files updated by Zhiting Zhu <zhitingz@cs.utexas.edu>
...
//!
//! ```rust
//! use lrucache::lrucache::LruCache;
//!
...
pub struct LruCache<K: Eq + Hash, V, S: BuildHasher = RandomState, M: CountableMeter<K, V> = Count>
{
    ...
    pub(crate) current_measure: M::Measure,
...
impl<K: Eq + Hash, V, M: CountableMeter<K, V>> LruCache<K, V, RandomState, M> {
    ...
    ///
    /// ```rust
    /// use lrucache::lrucache::{LruCache, Meter};
    /// use std::borrow::Borrow;
    ///
...
impl<K: Eq + Hash, V, S: BuildHasher, M: CountableMeter<K, V>> LruCache<K, V, S, M> {
    ...
    pub fn insert(&mut self, k: K, v: V) -> (Option<V>, Vec<(K, V)>) {
        ...
        if let Some(capacity) = self.capacity() {
            while self.size() > capacity {
                if let Some(entry) = self.remove_lru() {
                    evicted.push(entry);
                }
            }
        }
    ...
    /// assert_eq!(cache.get_mut(&3), Some(&mut "c"));
    /// ```
    pub fn set_capacity(&mut self, capacity: u64) -> Vec<(K, V)> {
        let mut evicted = Vec::new();
        while self.size() > capacity {
            if let Some(entry) = self.remove_lru() {
                evicted.push(entry);
